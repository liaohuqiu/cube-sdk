    ...
    public static <T> BaseRequestSender create(IRequest<T> request) {
        ...
        try {
            ...
            if (!requestData.shouldPost()) {
                sender = new GetRequestSender(request, urlConnection, requestData);
            } else {
                if (requestData.isMultiPart()) {
                    sender = new MultiPartRequestSender(request, urlConnection, requestData);
                } else {
                    sender = new PostRequestSender(request, urlConnection, requestData);
                }
            }
            ...
            request.setFailData(FailData.inputError());
        } catch (IOException e) {
