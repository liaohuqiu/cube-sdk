...
public class ImageProvider {
	...
	/**
	 * Return an {@link ImageProvider} instance. A {@link RetainFragment} is used to retain the ImageProvider object across configuration changes such as a change in device orientation.
	 * 
	 * @param fragmentManager
	 *            The fragment manager to use when dealing with the retained fragment.
	 * @param cacheParams
	 *            The cache parameters to use if the ImageProvider needs instantiation.
	 * @return An existing retained ImageProvider object or a new one if one did not exist
	 */
	public static ImageProvider getInstance(FragmentManager fragmentManager, ImageCacheParams cacheParams) {

		// Search for, or create an instance of the non-UI RetainFragment
		final RetainFragment mRetainFragment = findOrCreateRetainFragment(fragmentManager);

		// See if we already have an ImageProvider stored in RetainFragment
		ImageProvider imageCache = (ImageProvider) mRetainFragment.getObject();

		// No existing ImageProvider, create one and store it in RetainFragment
		if (imageCache == null) {
			imageCache = new ImageProvider(cacheParams);
	 ...
	 *            The bitmap to store
	 */
	public void saveBitmapToDiskCache(String key, Bitmap bitmap) {
	 ...
	 */
	public BitmapDrawable getBitmapFromMemCache(ImageRequest imageRequest) {
		BitmapDrawable memValue = null;

		if (mMemoryCache != null) {
			memValue = mMemoryCache.get(imageRequest.getCacheKey());
		}

		if (BuildConfig.DEBUG && memValue != null) {
			Log.d(TAG, String.format("%s Memory cache hit", imageRequest));
		}
	 ...
	 */
	public Bitmap getBitmapFromDiskCache(ImageRequest request) {
		Bitmap bitmap = null;
			...
			if (mDiskLruCache != null) {
				...
				try {
					...
					if (snapshot == null) {
						...
						if (editor != null) {
							if (ImageDownloader.downloadUrlToStream(request.getRemoteUrl(), editor.newOutputStream(DISK_CACHE_INDEX))) {
								editor.commit();
							} else {
								editor.abort();
							}
					...
					if (snapshot != null) {
						...
						if (inputStream != null) {
							...
							bitmap = ImageResizer.decodeSampledBitmapFromDescriptor(fd, request, this);
						}
					}
				} catch (final IOException e) {
	...
	/**
	 * A simple non-UI Fragment that stores a single Object and is retained over configuration changes. It will be used to retain the ImageProvider object.
	 */
